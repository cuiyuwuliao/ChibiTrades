...
rf_model_high_filedir = 'High_0_2024-05-01.pkl' #更改为模型文件 高价
rf_model_low_filedir = 'High_14_2024-05-01.pkl' #更改为模型文件 低价
rf_model_price_filedir = 'High_merged_2024-05-01.pkl' #更改为模型文件 收盘价
...
while True:
    ...
    Xdata_fit = Xdata[Xlistall[1:]]
    
    PredictNow['high_5m'] = (rf_model_high.predict(Xdata_fit)[0]+1)*current_price
    PredictNow['low_5m'] = (rf_model_low.predict(Xdata_fit)[0]+1)*current_price
    PredictNow['price_5m'] = (rf_model_price.predict(Xdata_fit)[0]+1)*current_price
    import numpy as np
    # 获取所有树的预测值
    model_high_tree_predictions = []
    for tree in rf_model_high.estimators_:
        tree_prediction = tree.predict(Xdata_fit)[0]
