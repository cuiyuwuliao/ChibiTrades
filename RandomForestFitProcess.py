...
#验证数据随机抽取时间戳：1713571140, 1714089540   即-7天到-1天
...
def train_and_evaluate_model(df, timestamps, Xlistall):
    r2_list = []
...
for i in range(6):
    random_seed = 42 + i
    timestamps = [random.Random(random_seed).randint(base_time_earlier1week, base_time_earlier1day) for _ in range(3)]
    thread = Thread(target=lambda ts=timestamps: r2_lists.append(train_and_evaluate_model(df, ts, Xlistall)))
...
# 合并r2_list并打印
r2_list_combined = np.concatenate(r2_lists)
print("所有模型训练并验证完成。")
